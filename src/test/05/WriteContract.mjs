...
const addressETH = '0x13586f74B4077c06Ea1B68ec41694d9BbFc2627F';

// 申明可写合约
const contractETH = new ethers.Contract(addressETH, abiETH, wallet);

const main = async () => {
  const decimals = await contractETH.decimals();
  console.log(`decimals: ${decimals}`);

  const ethBalance = await provider.getBalance(wallet.address);
  console.log(`ETH balance: ${ethers.formatUnits(ethBalance)}`);

  // 读余额
  const balance = await contractETH.balanceOf(
    '0xe8A335a8502625Fb6c6e900a547694770D764484'
  );
  console.log(`uniETH balance: ${ethers.formatUnits(balance)}`);

  // 执行mint操作，在之前需要先签名确认
  const tx = await contractETH.transfer(
    '0x0f59BfDEdbB4ECc965be28484BfD968552fD5C67',
    ethers.parseUnits('0.1', decimals)
  );
  await tx.wait();
  ...
  // 读余额
  const balance_transfer = await contractETH.balanceOf(
    '0xe8A335a8502625Fb6c6e900a547694770D764484'
  );
  console.log(
    `uniETH balance_transfer balance: ${ethers.formatUnits(balance_transfer)}`
  );
};
